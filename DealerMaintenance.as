...
[Bindable]
public var generalAC:ArrayCollection;
 ...
 */
private function getDealerRecord(value:Number = 0):void
{
    if ((dealerGrid.selectedIndex < 0) && (value == 0))
        return;
    var vID:Number;
    if (value > 0)
    {
    ...
    {
        vID = Number(dealerGrid.selectedItem.ID);
        _scrollPosition = dealerGrid.verticalScrollPosition;
        recordKey = dealerGrid.selectedItem.ID;
    }
    var csg:RemoteObject = new RemoteObject();
    csg.destination = "ColdFusion";
...
private function lookupNameRO(event:Event):void
{
    ...
    {
        case "btn_Find":
            if ((search_fld.text.length > 2) && (fld_ID.length == 0))
            {
                state_cb.selectedIndex = 0;
                lookupValue = search_fld.text;
                fld_ID.text = '';
    ...
    switch (act.message.operation)
    {
        ...
        case "saveDealer":
            ...
            if (action == "Update")
            {
                ...
                {
                    generalAC.removeItemAt(lastEditRecord);
                    generalAC.addItemAt(dealerRO, lastEditRecord);
                        // locateAndSelectRecord();
                }
            }
            else if (action == "Add")
            {
...
private function locateAndSelectRecord():void
{
    if (recordKey == -1)
        return;
    var cursor:IViewCursor = ArrayCollection(dealerGrid.dataProvider).createCursor();
    while (!cursor.afterLast)
    {
        if ((cursor.current.ID == recordKey))
        {
            dealerGrid.selectedItem = cursor.current; // if we found the record make it selected in the grid
            dealerGrid.verticalScrollPosition = _scrollPosition; // and scroll to that position so we can see where we were
            break;
        }
        else
        {
            cursor.moveNext(); // if we can't get the next record we must be at the end.
        }
    }
    recordKey = -1; // reset the ppid marker
    _scrollPosition = 0; // reset the scroll position
}

private function getRecord():void
{
    getDealerRecord(Number(this.fld_ID.text));
    lastEditRecord = -1;
    this.fld_ID.text = ''
    search_fld.text = '';
    state_cb.selectedIndex = 0;
